...
def unorderedRunnerAux {χ : Type _} [Ord χ] (inputs : Array α) (f : α → β) (numIter : Nat)
  (order : α → χ) : IO $ Batteries.RBMap χ Nat compare := do
    let mut cron : Batteries.RBMap χ Nat compare := .empty
...
open Std in
def orderedRunnerAux [Ord α] (inputs : Array α) (f : α → β) (numIter : Nat)
  : IO $ Batteries.RBMap α Nat
